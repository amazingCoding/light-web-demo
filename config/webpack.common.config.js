...
const getCommonConfig = (__DEV__) => {
  ...
  routers.map((item) => {
    // 每个页面使用一个entry配置
    const routerScript = __DEV__ ? ['webpack-hot-middleware/client?reload=true', path.join(appDir, `router/${item}/index.tsx`)] : [path.join(appDir, `router/${item}/index.tsx`)]
